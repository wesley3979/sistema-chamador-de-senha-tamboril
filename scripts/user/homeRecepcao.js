      ...
      $("#tableFilaAtendimento tbody").append(
        `<tr>
                    ...
                    <td class="d-flex justify-content-around">
                        <a title="Cancelar senha" onClick="setIdModal(${
                          senha.id
                        })" href="" style="text-decoration: none;" data-toggle="modal" data-target="#cancelarSenhaModal">
                            <i class="fas fa-times" style="color: #dc3545;"></i>
                        </a>
                        <a title="Imprimir senha" onClick="ImprimirSenhaById(${
                          senha.id
                        })" href="#" style="text-decoration: none;">
                            <i class="fas fa-print" style="color: #138496;"></i>
                        </a>
                    </td>
...
socket.on("senhas canceladas", (senhas) => {
  $("#tableHistoricoAtendimentoCancelado tbody").html("");

  let index = 0;
  senhas.forEach((senha) => {
    if (senha.Setor.UbsId == $("#userUbsId").val()) {
      $("#tableHistoricoAtendimentoCancelado tbody").append(
        `<tr>
                    <th scope="row">${index + 1}°</th>
                    <td>${senha.Numero}</td>
                    <td>${senha.Paciente}</td>
                    ...
                    <td>${senha.Preferencial ? "Sim" : "Não"}</td>
                    <td>
                        <i class="fas fa-solid fa-circle" style="color: #dc3546;"></i>
                        Cancelado
                    </td>
                </tr>`
      );
  ...
  var rowCount = $("#tableHistoricoAtendimentoCancelado tbody tr").length;
  $("#quantidadeSenhaFinalizada").html(rowCount);
...
function CadastrarSenha() {
  $("#cadastrarSenhaForm").submit(function (event) {
    ...
    if ($("#preferencial").is(":checked")) preferencial = 1;
...
function EditMyUser() {
  $("#editMyUserForm").submit(function (event) {
    ...
    const lastName = removeSpecialChars(
      $("#myUserLastName").val().toLowerCase()
    );
...
function ImprimirSenhaById(id) {
  loadPageAnimation(true);

  $.ajax({
    type: "GET",
    url: "/" + $("#userUbsId").val() + "/senha/getSenhaById/" + id,
    error: function (error) {
      loadToastNotification(
        "Erro interno, estamos solucionando o problema, tente fazer login novamente em instantes",
        "danger"
    ...
    success: function (result) {
      if (result.status === "success") {
          $("#nomeImpressao").text(` ${result.senha.Paciente}`);
          $("#setorImpressao").text(` ${result.senha.Setor.Nome}`);
          $("#senhaImpressao").text(` ${result.senha.Numero}`);
